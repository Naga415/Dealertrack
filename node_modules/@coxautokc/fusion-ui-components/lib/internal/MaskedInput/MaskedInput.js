...
var TextInput = function (_Component) {
  ...
  _createClass(TextInput, [{
    ...
    // NOTE: Why are we implementing shouldComponentUpdate?
    // 1. It avoids unnecessary renders.
    // 2. There is a known issue on React Bootstrap's modal that causes cursor jump:
    //    https://github.com/react-bootstrap/react-bootstrap/issues/2323.
    //    Implementing shouldComponentUpdat resolves the issue. React 16's Portal
    //    feature will allow this issue to be addressed:
    //    https://github.com/facebook/react/issues/5990
    //    However, React Bootstrap isn't ready for 16 yet.
    // 3. Even when issue in #2 above is resolved, we can keep this function for performance reasons.
    ...
    value: function shouldComponentUpdate(nextProps, nextState) {
      ...
      // loop over object and track what is different.
      var diffs = Object.entries(nextProps).reduce(function (diffs, _ref) {
        var _ref2 = _slicedToArray(_ref, 2),
            key = _ref2[0],
            value = _ref2[1];

        if (value !== _this2.props[key]) {
          diffs.push(key);
          return diffs;
        }
        return diffs;
      ...
      // if nothing is different, don't rerender.
      if (diffs.length === 0) {
        return false;
      }

      // if only error has changed, don't rerender.
      if (diffs[0] === 'error' && diffs.length === 1) {
        return false;
      }
