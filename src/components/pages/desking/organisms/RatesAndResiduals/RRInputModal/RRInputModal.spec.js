...
import {RRInputModalClass as RRInputModal} from './RRInputModal';
import { shallow } from 'enzyme';
import sinon from 'sinon';
import ProgramsTable from './ProgramsTable';
import Lenders from './Lenders';
import Footer from './IncentivesTable';
...
const onRRModalHide = sinon.spy();

function setupProps(lenders, programs, selectedProgram, incentives, selectedIncentives){
  return {
    lenders,
    programs,
    selectedProgram,
    ...
    selectedIncentives,
    show: true,
    onHide: onRRModalHide
  };
}

function setupshallow (lenders, programs, selectedProgram, incentives, selectedIncentives) {
  const props = setupProps(lenders, programs, selectedProgram, incentives, selectedIncentives);
  return shallow(<RRInputModal {...props} />);
}

describe('RRInputModal', ()=>{
  it('should render <RRInputModal /> component', () => {
    const component = setupshallow(lenders, programs);
    expect(component.find(Lenders).length).toBe(1);
    expect(component.find(ProgramsTable).length).toBe(1);
