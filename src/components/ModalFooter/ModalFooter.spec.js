...
describe("ModalFooter specs", () => {
  ...
  it("triggers refresh button when click", () => {
    const component = mount(<ModalFooter {...props} />);
    component.find("#modalName-ModalReset").simulate("click");
    expect(props.resetData.calledOnce).toEqual(true);
  });

  it("refresh button not exists when pass false as displayResetBtn", () => {
    props["displayResetBtn"] = false;
    const component = mount(<ModalFooter {...props} />);
    expect(component.find("#modalName-ModalReset").exists()).toEqual(false);
  });
});
